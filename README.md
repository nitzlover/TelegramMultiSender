...
- **2FA**: Логика обработки двухфакторной аутентификации (запрос пароля при QR‑авторизации).
- **Отдельное окно «Дебаг»**: Отдельное окно «Дебаг» в отдельный пункт меню и вывод лога в новое окно.
- **Задержка (SpinBox)**: Настройка задержки между отправками для обхода FloodWait и блокировок.
